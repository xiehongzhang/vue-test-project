<template>
  <div>
    <template v-if="show">
      <vue-aliplayer-v2 ref="vueAliplayerV2" :source="value" :options="options" @ready="handleReady" />
    </template>
    <footer>
      <!-- 菜单栏 -->
      <div class="menu">
        ...
        <div class="videoContorl">
          ...
          <!-- <i class="iconfont icon-kuaijin-1" @click="prevPage" />
      ...
      <div class="controlLine">
        <div v-if="signFlag" class="signshowImg" :style="`left:${signLeft}`">
          <span class="signDetail icon-qingchu iconfont" @click="signDelete" />
          <span class="signClose icon-chuyidong iconfont" @click="signFlag = false" />
          <div class="icon-xiugai iconfont text" @click="changeSignText">{{ signText }}</div>
        </div>
        <div id="pickeddeng" class="dyc">
          <div class="canFa" @mouseup="blueBgUp">
            <!-- 画布 -->
            <canvas id="canvas" :width="canvasWidth" height="80" @mousemove="showMoveImg" />
            <div
            ...
            />
            <div id="blueBg" ref="timeMove" class="blueBg" @mousedown="blueBgDown" @mousemove="blueBgMove" @mouseup="blueBgUp">
              {{ timeCurrentLeft }}
              <span class="turnDowm" />
            </div>
          </div>
    ...
    >
      ...
      <el-table v-if="radio===1" :data="tableData" style="width: 100%">
        <el-table-column type="index" label="拆条列表" width="200" align="center" />
        <el-table-column prop="timeLong" label="时长" align="center" />
    ...
    <el-dialog :title="spliceMsg" :visible.sync="dialogVisible" width="600" append-to-body>
      ...
      <div class="wenjianBox" style="height:160px;overflow:auto;">
        <el-tree :data="wenjianList" accordion @node-click="handleNodeClick" />
      </div>
      <el-table :data="cutCoverList" style="width: 100%">
        <el-table-column label="名称" align="center" show-overflow-tooltip prop="text" />
        <el-table-column label="开始时间" align="center" show-overflow-tooltip prop="startTime" />
        <el-table-column label="结束时间" align="center" show-overflow-tooltip prop="endTime" />
        <el-table-column label="分段时长" align="center" show-overflow-tooltip prop="timeLong" />
...
export default {
  ...
  methods: {
    // 播放器视频初始化按钮渲染完毕后的操作
    handleReady(e){
      // console.log(this.$refs.vueAliplayerV2.getDuration())
      const duration = this.$refs.vueAliplayerV2.getDuration()
      if (duration){
        this.initVideoTime(duration)
      }
    },
    // 初始化组件的相关信息，包括视频时长，vancas的绘画
    initVideoTime(duration){
    ...
    weitiao(index, flag1, flag2){
      ...
      if (flag1 === 1){
        ...
        const fininal = parseFloat(currentCut.left) - 40
        ...
        const fininal = (parseFloat(currentCut.left) + parseFloat(currentCut.width) - 40)
        this.timeCurrentLeft = this.getStartEndTime(fininal + 40)
        this.$emit('currentTime', this.timeCurrentLeft)
        // 调整结束位置
        if (flag2 === 1){
    ...
    faPKMove($event) {
      ...
      if (this.turnFlag === 'left'){
        ...
        const finwidth = finright - finleft
        ...
        // 设置宽度
        const finwidth = finleft - curLeft + 30
    ...
    subSection(value) {
      ...
      this.$emit('subSectionPlay', value)
    ...
    setLeftAndStart(index, $event) {
      const currentBox = this.cutCoverList[index]
      const left = parseFloat(currentBox.left)
      const finleft = left + ($event.offsetX - this.offsetStart)
      currentBox.left = finleft + 'px'
    ...
    stepChange() {
      ...
      // const moveLeft = parseFloat(document.getElementById("blueBg").style.left);
      switch (this.value2) {
        case 0:
          this.number = 600
          break
    ...
    drawCan(cxt, config, number) {
      ...
      // const myj=0
      // for(const i=0;i<this.maxTimeLong;i+=this.number){
      //   myj++;
    ...
    onControl(index) {
      ...
      switch (index) {
        ...
        case 2: // 视音频分离
          ...
          break
        case 3: // 快速选段
          moveLeft = document.getElementById('blueBg')
          this.moveLeft = window.getComputedStyle(moveLeft).left
          console.log(this.moveLeft)
          width = (this.quickChoseTime / this.number) * 100 + 'px'
          this.$set(this.cutCoverList, this.cutCoverList.length, {
            clickFlag: true,
    ...
    autuEnsure() {
      // const timeMove = document.getElementById('blueBg')
      // const left = parseFloat(window.getComputedStyle(timeMove).left) + 40
      if (this.radio === 1) {
        // 自定义拆条
        const timeLong = []
        ...
        const setAutoList = []
        for (let i = 0; i < timeLong.length; i++) {
          let prev = null
          if (i - 1 >= 0) {
          ...
          const width = (timeLong[i] / this.number) * 100 + 'px'
          const left = parseFloat(prev.left) + parseFloat(prev.width) + 'px'
          setAutoList.push({
            left,
            width,
            clickFlag: true,
        ...
        for (let i = 0; i < setAutoList.length; i++) {
          this.$set(
            this.cutCoverList,
        ...
        const autoWidth = (this.autoNum / this.number) * 100
        const maxNumber = Math.floor(parseFloat(this.imgWidth) / autoWidth)
        const setAutoList = []
    ...
    entureChaitiao() {
      // const curVideo = this.$store.state.curVideo
      // console.log(this.$store.state.curVideo);
      const optionTimes = []

      // if (this.numberFlag === "01") {
      //   //  拆分
      //   for (const i = 0; i < this.cutCoverList.length; i++) {
      //     const Lstart = this.cutCoverList[i].startTime.split(".")[0];
      //     const Lend = this.cutCoverList[i].endTime.split(".")[0];
      //     const Rstart = String(
      ...
      // return
      if (this.cutCoverList.length <= 0){
        return this.$message.info('还没有选择截取的时间')
      }
      // 遍历截取视频的数组
      for (let i = 0; i < this.cutCoverList.length; i++){
        const startTime = this.transformTime(this.cutCoverList[i].startTime.split('.')[0])
        const endTime = this.transformTime(this.cutCoverList[i].endTime.split('.')[0])
        optionTimes.push({
      ...
      // 开始截取视频
      const data = {
        startOffset: optionTimes[0].startOffset,
        endOffset: optionTimes[0].endOffset,
        url: this.value
      }
      cutVideo(data).then(response => {
        if (response.data.code !== 200){
          return this.$message.error('视频剪切失败')
        }
        this.$message.success('视频剪切成功')
